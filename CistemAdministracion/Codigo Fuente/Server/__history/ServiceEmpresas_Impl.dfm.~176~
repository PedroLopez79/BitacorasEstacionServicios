        ...
        Statements = <
          item
            ...
            SQL = 
              ...
              'ZARMANTENIMIENTO ASC'#10'FOR XML RAW ('#39'SERVICES'#39'), ELEMENTS)'#10'END'#10#10'SE' +
              'LECT CAST(@COUNT AS VARCHAR(100)) + @XML AS XML'
            StatementType = stSQL
            ColumnMappings = <
              item
                DatasetField = 'XML'
        ...
        Statements = <
          item
            ...
            SQL = 
              ...
              'ESTACION = :EstacionID AND EMPLEADO.TURNO = :TURNO'#10#10'DECLARE CMIE' +
              'MPLEADO CURSOR FOR'#10'SELECT     IDEMPLEADO'#10'FROM         #HORARIOEM' +
              'PLEADO'#10'OPEN CMIEMPLEADO'#10'FETCH NEXT FROM CMIEMPLEADO INTO @IDEMPL' +
              'EADO'#10'    WHILE (@@FETCH_STATUS = 0)'#10#9'BEGIN'#10#9#9'SET @FOTOEMPLEADO =' +
              ' (SELECT #HORARIOEMPLEADO.FOTOEMPLEADO FROM #HORARIOEMPLEADO WHE' +
              'RE #HORARIOEMPLEADO.IDEMPLEADO = @IDEMPLEADO)'#10#10#9#9'SET @FOTOBASE64' +
              ' = (SELECT '#10'                          CAST('#39#39' AS XML).value('#39'xs:' +
              'base64Binary(sql:variable("@FOTOEMPLEADO"))'#39','#39'VARCHAR(MAX)'#39') AS ' +
              'FOTOB64'#10'                          FROM #HORARIOEMPLEADO'#10'        ' +
              '                  WHERE #HORARIOEMPLEADO.IDEMPLEADO = @IDEMPLEAD' +
              'O)'#10#10'        UPDATE #HORARIOEMPLEADO'#10#9#9'SET FOTOBASE64 = @FOTOBASE' +
              '64 WHERE #HORARIOEMPLEADO.IDEMPLEADO = @IDEMPLEADO'#10#10#9#9'SET @FOTOE' +
              'MPLEADO = NULL'#10#9#9'FETCH NEXT FROM CMIEMPLEADO INTO @IDEMPLEADO   ' +
              '   '#10'    END'#10'    CLOSE CMIEMPLEADO'#10'    DEALLOCATE CMIEMPLEADO '#10#10'D' +
              'ECLARE @XML VARCHAR(MAX)'#10#10'SET @XML = (SELECT #HORARIOEMPLEADO.ID' +
              'EMPLEADO, #HORARIOEMPLEADO.HORAINICIO, #HORARIOEMPLEADO.HORAFINA' +
              'L, #HORARIOEMPLEADO.IDESTACION,'#10'       #HORARIOEMPLEADO.NOMBREEM' +
              'PLEADO, #HORARIOEMPLEADO.REFERENCIA, #HORARIOEMPLEADO.FOTOBASE64' +
              ' FROM #HORARIOEMPLEADO'#10'FOR XML RAW ('#39'SERVICES'#39'), ELEMENTS)'#10#10'SELE' +
              'CT CAST(@COUNT AS VARCHAR(100)) + @XML AS XML'#10#10'DROP TABLE #HORAR' +
              'IOEMPLEADO'
            StatementType = stSQL
            ColumnMappings = <
              item
