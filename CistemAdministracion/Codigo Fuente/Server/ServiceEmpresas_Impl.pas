...
{$ENDIF}
uses
  {Generated:} LibraryEmpresas_Invk, fServerDataModule, UtileriasComun;
...
function TServiceEmpresas.AbreDataSetReporte(const SQL: AnsiString;
...
{procedure TServiceEmpresas.ActualizaCupon(const Barras, NewParam: String;
...
{procedure TServiceEmpresas.ActualizaCupon(const Barras: String;
  const DiaConsumo, EjercicioConsumo: Integer; const FechaConsumo: DateTime;
...
procedure TServiceEmpresas.ActualizaCupon(const Barras: AnsiString;
  const DiaConsumo, EjercicioConsumo: Integer; const FechaConsumo: DateTime;
...
procedure TServiceEmpresas.ActualizaCuponConReferencia(
...
function TServiceEmpresas.ActualizaLiquidacionProd(
...
procedure TServiceEmpresas.ActualizaStatusCupon(const Barras: AnsiString;
  const DiaConsumo, EjercicioConsumo: Integer; const FechaConsumo: DateTime;
...
function TServiceEmpresas.AfectaInventarios(const LiquidacionID,
...
function TServiceEmpresas.AgrupacionesBomba(
...
function TServiceEmpresas.BuscaDespachadorLiquidacion(const BombaID,
...
function TServiceEmpresas.BuscaOtroProducto(const EstacionID: Integer;
...
function TServiceEmpresas.BuscaReporte(const Nombre: AnsiString;
...
procedure TServiceEmpresas.CalculaIEPS(var Datos: TDatosFactura);
var
  Aux, Impuesto: Double;
  SubTotal: Double;
  I: Integer;
...
procedure TServiceEmpresas.CalculaIEPSExpress(var Datos: TFacturaExpress);
var
  Aux, Impuesto: Double;
  SubTotal: Double;
  I: Integer;
...
procedure TServiceEmpresas.CalculaISR_IVARTN(var Datos: TDatosFactura);
var
  I: Integer;
  ds: IDADataset;
...
function TServiceEmpresas.PuntosCalculaPuntos(const ClienteID, ProductoID: Integer;
...
function TServiceEmpresas.PuntosCalculaSaldo(
...
function TServiceEmpresas.PuntosDatos(
...
procedure TServiceEmpresas.PuntosDespachador(const TurnoID, EstacionID,
  UsuarioID: Integer);
var
  ds: IDADataSet;
  cmd:IDASQLCommand;
...
function TServiceEmpresas.CambiaFolioFactura(const Serie: AnsiString;
...
function TServiceEmpresas.CancelaFactura(const Folio: Integer;
...
function TServiceEmpresas.CER(const texto, Dir: AnsiString): string;
var
  F:TextFile;
  S: String;
begin
...
function TServiceEmpresas.CERNUM(const texto, Dir: AnsiString): String;
var
  F:TextFile;
  S: String;
  serial:String;
...
function TServiceEmpresas.CierraLiquidacion(
...
function TServiceEmpresas.ClienteValido(
...
function TServiceEmpresas.CostoProducto(
...
procedure TServiceEmpresas.DataAbstractServiceCreate(Sender: TObject);
begin
   CoInitialize (nil);
end;

function TServiceEmpresas.DatosCerrarLiquidacion(const EstacionID,
...
function TServiceEmpresas.DatosCliente(const ClienteID: Integer;
...
function TServiceEmpresas.DatosFacturaElectronica(const FacturaID,
...
function TServiceEmpresas.DatosPremio(
...
function TServiceEmpresas.ValidaConsumo(
...
function TServiceEmpresas.ValidaFolioFactura(const Serie: Ansistring;
...
function TServiceEmpresas.ValidaLiquidacionDespachador(
  const LiquidacionID: Integer): AnsiString;
var
  ds: IDADataSet;
begin
...
function TServiceEmpresas.ValoresTurno(const EstacionID,
...
procedure TServiceEmpresas.dbpLiquidacionBeforeProcessDelta(
  Sender: TDABusinessProcessor; const aDelta: IDADelta);
begin
  Exit;
end;

function TServiceEmpresas.EliminaAutomaticosLiquidacion(const EstacionID,
...
function TServiceEmpresas.EntregaPremio(const ClienteID, PremioID,
...
function TServiceEmpresas.Existencia(const EstacionID, AlmacenID,
...
function TServiceEmpresas.FacturaExpress(const Serie: Ansistring;
...
function TServiceEmpresas.FacturaID(const Serie: AnsiString;
...
function TServiceEmpresas.FacturaYLiquidacion(
...
function TServiceEmpresas.Folio(const Campo, Serie: AnsiString): Integer;
var
  ds: IDADataSet;
begin
...
function TServiceEmpresas.FolioActual(const Campo,
...
function TServiceEmpresas.FolioActual2(const Campo, Serie: AnsiString;
...
function TServiceEmpresas.GeneraFactura(const EstacionID: Integer;
  const Serie: Ansistring; const FechaCorte: TDateTime; const ClienteID,
...
procedure TServiceEmpresas.GuardaAccesos(const UsuarioID: Integer;
...
function TServiceEmpresas.GuardaArchivo(nomarch,Texto: String): Integer;
var
  F: TextFile;
begin
    {$I-}
...
function TServiceEmpresas.GuardaConsumoExpress(
...
procedure TServiceEmpresas.GuardarDatosFacturaPemex(
...
function TServiceEmpresas.registraactividadrealizada(const IDACTIVIDADMANTENIMIENTO: Integer;
...
procedure TServiceEmpresas.GuardaLimiteFactura(const UsuarioID: Int64;
...
procedure TServiceEmpresas.GuardarDatosFactura(
...
procedure TServiceEmpresas.GuardarDatosLiquidacion(
...
procedure TServiceEmpresas.GuardarDatosReciboPago(
...
function TServiceEmpresas.GuardarFacturaExpress(const ClienteID: Integer;
...
function TServiceEmpresas.IEPS(ProductoID: Integer): Real;
var
  ds: IDADataset;
begin
  ds:=Schema.NewDataset(Connection, 'spIEPS');
...
procedure TServiceEmpresas.InsertaDeposito(const DepositoID: Integer;
...
procedure TServiceEmpresas.InsertaProductoPrecio(const ProductoID,
...
function TServiceEmpresas.LiquidacionCerrada(const EstacionID,
  TurnoID: Integer): Boolean;
var
  ds: IDADataSet;
begin
...
function TServiceEmpresas.LlavePriv(const Password,
...
function TServiceEmpresas.LlavePrivaCertificado(const LlavePrivada,
...
function TServiceEmpresas.Login(const Usuario, Clave: AnsiString): TLoginInfo;
var
...
function TServiceEmpresas.ModificarFolioActual(const Campo, Serie: AnsiString;
...
function TServiceEmpresas.obtendatosActividadesProgramadas(const IDESTACION,
...
function TServiceEmpresas.obtendatosempleados(
...
function TServiceEmpresas.ObtenerTipoCambioIDPorEstacion(
  const EstacionID: Integer): Integer;
var
  ds: IDADataSet;
begin
...
function TServiceEmpresas.ObtenerTipoCambioPorEstacion(
...
function TServiceEmpresas.obtenNotificacionesDiarias(
...
function TServiceEmpresas.PrecioProducto(const EstacionID,
...
function TServiceEmpresas.PremioPuntos(EstacionID: Integer; Puntos: Double): String;
var
  ds: IDADataset;
begin
  Result := '';
...
procedure TServiceEmpresas.ProcesaVentasLiquidacion(
...
procedure TServiceEmpresas.ProgramaMantenimiento(const IDPROGRAMAMANTENIMIENTO: Integer; const IDESTACION: Integer; const FECHA: DateTime);
var
  cmd: IDASQLCommand;
begin
...
function TServiceEmpresas.StatusTicket(const EstacionID,
...
function TServiceEmpresas.SumaAnticipo(const Estacion,Secuencia: Integer):Double;
var
  ds: IDADataSet;
begin
...
function TServiceEmpresas.SumaVentasCorte(const Estacion,
...
function TServiceEmpresas.TicketsLiquidacion(
  const LiquidacionID: Integer): Ansistring;
var
  Aux: TStrings;
  ds: IDADataset;
...
function TServiceEmpresas.TurnoALiquidacionID(const TurnoID,
...
initialization
  TROClassFactory.Create('ServiceEmpresas', Create_ServiceEmpresas, TServiceEmpresas_Invoker);
finalization
end.
